...
public sealed class GameEventBusLogTests
{
    ...
    {
        var bus = new GameEventBus(10, new[] { EventTypes.CombatStarted });

        var initialVersion = bus.RecentVersion;
        bus.Publish(new GameEvent("evt-1", EventTypes.CombatStarted, "run-1", 1, "Combat", new { }));
    ...
    {
        var bus = new GameEventBus(10, new[] { EventTypes.CombatStarted, EventTypes.PlayerEnergyChanged });

        bus.Publish(new GameEvent("evt-1", EventTypes.CombatStarted, "run-1", 1, "Combat", new { enemyCount = 2 }));
