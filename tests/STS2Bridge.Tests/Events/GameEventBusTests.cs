...
public sealed class GameEventBusTests
{
    ...
    {
        var bus = new GameEventBus(2, new[] { EventTypes.PlayerDamaged, EventTypes.PlayerEnergyChanged });
        GameEvent? received = null;
        bus.Subscribe(evt => received = evt);

        var first = new GameEvent("evt-1", EventTypes.PlayerDamaged, "run-1", 1, "MonsterRoom", new { amount = 2 });
